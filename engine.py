...
def _evaluate_btts(stats: dict[str, dict[str, float]], rule: dict[str, Any]) -> Recommendation:
    ...
    assert btts and team_scored and under25 and goals and clean_sheets
    ...
    reasons = [
        ...
        f"Dominacja {side_label}: gole {dominant_goals:.2f} vs {weaker_goals:.2f}, różnica {goal_gap:.2f}, czyste konta {dominant_clean_sheets:.1f}%",
        f"Wyjątek strony {weaker_label}: Team scored {weaker_team_scored:.1f}%/{dominance_escape_team_scored:g}, BTTS {weaker_btts:.1f}%/{dominance_escape_btts:g}",
        f"Blok dominacji jednostronnej: {'TAK' if dominance_profile and not dominance_escape else 'NIE'}",
